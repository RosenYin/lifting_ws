...
class C_ROS_Server:
    def __init__(self,) -> None:
        ...
        初始化节点
        '''
        rospy.init_node('lifting_ctrl_850pro_server_node', anonymous=True)
        # 外部参数,判断控制哪一个电机编号
        self.motor_id = 1
    ...
    def SververCallbackImmediately(self,req):
        '''
        850Pro 服务端回调函数,立刻反馈数值，不等待电机到达位置就反馈
        '''
        self.input_callback_time = time.time()
        self.command_send = 1
        ...
        if(self.callLock == self.last_callLock):
            ...
            # 恒定速度
                ...
                if(self.target_speed < -abs(self.liftTargetSpd)): self.target_speed=-abs(self.liftTargetSpd)
                rospy.loginfo("----------操作电机 %d 最终发送速度数值为%d",self.motor_id, 
                              self.ctrl.ConvertSendRealSpdVal(self.ctrl.LiftSpdConvertToMotorSpd(self.target_speed)))
                if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                    rospy.loginfo(self.motor_states.back_speed)
                    if(abs((self.ctrl.CalLiftSpd(self.motor_states.back_speed)) - self.target_speed)<20):
                        resp = 1
                else: resp = -1
                ...
                if(self.target_speed < -self.liftTargetSpd): self.target_speed=-self.liftTargetSpd
                self.target_speed = self.ctrl.LiftSpdConvertToMotorSpd(self.target_speed)
            ...
            # 恒定速度
                ...
                if(self.target_speed < -max_spd): self.target_speed=-max_spd
                rospy.loginfo("----------操作电机 %d 最终发送速度数值为%d",self.motor_id, self.ctrl.ConvertSendRealSpdVal(self.target_speed))
                if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                    rospy.loginfo(self.motor_states.back_speed)
        ...
        if(self.callLock == self.last_callLock):
            while resp == -1:
                ...
                # 恒定速度
                    ...
                    if(self.target_speed > abs(self.liftTargetSpd)): self.target_speed=abs(self.liftTargetSpd)
                    if(self.target_speed < -abs(self.liftTargetSpd)): self.target_speed=-abs(self.liftTargetSpd)
                    rospy.loginfo("----------操作电机 %d 最终发送速度数值为%d",self.motor_id, self.target_speed, 
                                  self.ctrl.ConvertSendRealSpdVal(self.ctrl.LiftSpdConvertToMotorSpd(self.target_speed)))
                    if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                        rospy.loginfo(self.motor_states.back_speed)
                        if(abs((self.ctrl.CalLiftSpd(self.motor_states.back_speed)) - self.target_speed)<20):
                ...
                # 恒定速度
                    ...
                    if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                        rospy.loginfo(self.motor_states.back_speed)
                        if(abs(self.motor_states.back_speed - self.target_speed)<15):
                            resp = 1
                    else: resp = -1
                else:
                    resp = 0
