...
class C_LiftingMotorCtrl_850pro():
    class motor_msg():
        ctrl_mode:int
        back_vol:int
        back_current:float
            ...
            for l in workable_port_list :
                if(l[:11] == '/dev/ttyUSB' and l!=ignore_port):
                    ...
                    for i in range(10):
                        ...
                        if(rxdata is not None and len(rxdata)==4):
                            is_cur_port = True
                        if(is_cur_port == True):
                            # print(rxdata)
    ...
    def ReadMotorLimitData(self):
        '''
        读取限位开关消息，0位是上限位，1位是下限位,850与830不同
        '''
        # 01 03 00 58 00 01 05 D9 #读限位
        txlist = [self.__motor_id, 0x03, 0x00, 0x54, 0x00, 0x01]
    ...
    def LiftLimitInit(self, motor_speed:int, motor_state:motor_msg):
        ...
        运动升降柱指定速度来用限位来初始化位置
        '''
        if not isinstance(motor_state, C_LiftingMotorCtrl_850pro.motor_msg):
            return False
        limit:bool
    ...
    def MotorStop(self, direction, motor_state:motor_msg, force:bool=False):
        ...
        if(force):
            ...
            if(direction==1):
                if isinstance(motor_state, C_LiftingMotorCtrl_850pro.motor_msg):
